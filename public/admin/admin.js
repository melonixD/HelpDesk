(function () {
  ...
  function photoSource(value) {
    return HelpDeskAssets.assetUrl(value || "/favicon.svg", window.location.origin);
  }

  function notifyLiveSite() {
    try { localStorage.setItem("helpdesk-content-updated", `${Date.now()}-${Math.random()}`); } catch {}
  }
  ...
  async function loadDashboard() {
    ...
    state.data = { resources: HelpDeskSyllabus.ensure(payload.resources), placements: payload.placements, notices: payload.notices, scholarships: payload.scholarships };
    ...
    $("#admin-avatar").src = photoSource(state.profile.photoUrl);
  ...
  $("#login-form").addEventListener("submit", async (event) => {
    ...
    try {
      const result = await request("/api/admin/login", { method: "POST", body: JSON.stringify({ username: $("#login-username").value, password: $("#login-password").value, rememberMe: $("#login-remember").checked }) });
  ...
  async function saveChanges() {
    ...
    try {
      ...
      if (!state.dirty) {
        // Save returns a merged document. Bind inputs to that document before
        // the next edit; old listeners otherwise mutate the discarded copy.
  ...
  async function publishSavedDraft(key, label) {
    ...
    try {
      ...
      renderHistory();
      notifyLiveSite();
      toast("Published through Netlify Blobs. The live site will refresh when you return to its tab.");
      return true;
    } catch (error) {
  ...
  function bind(container, root) {
    ...
    $$('[data-upload-bind]', container).forEach((node) => node.addEventListener("change", async () => {
      ...
      try {
        ...
        bound.value = url; bound.dispatchEvent(new Event("input", { bubbles: true })); toast(state.section === "creators" ? "Photo uploaded to your draft. Click Save draft, then Publish changes." : "Upload complete.");
  ...
  async function uploadFile(file, previousUrl, onProgress) {
    ...
    for (let attempt = 0; attempt < 3 && !verified; attempt += 1) {
      ...
      const response = await fetch(photoSource(result.url), { method: "HEAD", credentials: "same-origin", cache: "no-store" });
  ...
  function renderYearWisePyqsEditor(subject) {
    return `<section class="editor-resource-block year-pyqs-editor"><div class="resource-block-head"><div><p class="resource-kicker">Previous year papers</p><h3>Year-Wise-PYQs</h3><span>Open a year and paste its Google Drive links. Empty papers appear as Coming soon. Save your draft, then publish when ready.</span></div></div>
      ${subject.yearWisePyqs.map((year, index) => { const active=HelpDeskYearPyqs.activePapers(year); const missing=HelpDeskYearPyqs.papers.filter(([field])=>!year.enabledPapers.includes(field)); return `<details class="unit-card" ${state.selection.yearPyqId === year.id ? "open" : ""}><summary class="unit-summary"><strong>${escape(year.year)}</strong><span class="resource-total">${active.filter(([field]) => HelpDeskYearPyqs.paperUrl(year[field])).length} / ${active.length} ready</span></summary><div class="unit-body"><div class="grid">${input("Year", `yearWisePyqs.${index}.year`, year.year, {type:"number", required:true})}${active.map(([field, label]) => `<div class="pyq-paper-editor">${input(label + " · Google Drive link", `yearWisePyqs.${index}.${field}`, year[field] || "", {type:"url",full:true,help:"Leave blank for Coming soon."})}<button type="button" class="mini-button danger" data-remove-pyq-paper="${index}:${field}">Remove ${escape(label)}</button></div>`).join("")}</div>${active.length?"":`<p class="resource-empty">No exam sections in this year. Add one below whenever needed.</p>`}${missing.length?`<div class="pyq-paper-add"><label class="field"><span>Add exam section</span><select data-add-pyq-paper-select="${index}">${missing.map(([field,label])=>`<option value="${field}">${escape(label)}</option>`).join("")}</select></label><button type="button" class="mini-button" data-add-pyq-paper="${index}">＋ Add section</button></div>`:""}<button type="button" class="mini-button danger" data-remove-pyq-year="${index}">Remove year</button></div></details>`; }).join("")}
  ...
  function bindYearWisePyqs() {
    $$("[data-remove-pyq-paper]").forEach(button => button.addEventListener("click", () => {
      const [rawIndex, field] = button.dataset.removePyqPaper.split(":"); const index = Number(rawIndex);
      const subject = selectedSubject(); const year = subject.yearWisePyqs[index];
      const paper = HelpDeskYearPyqs.papers.find(([key]) => key === field); if (!paper) return;
      if (!confirm(`Remove ${paper[1]} from ${year.year}? You can add it back later.`)) return;
      year.enabledPapers = year.enabledPapers.filter(key => key !== field); year[field] = "";
      state.selection.yearPyqId = year.id; markDirty(); renderResources();
    }));
    $$("[data-add-pyq-paper]").forEach(button => button.addEventListener("click", () => {
      const index = Number(button.dataset.addPyqPaper); const subject = selectedSubject(); const year = subject.yearWisePyqs[index];
      const select = $(`[data-add-pyq-paper-select="${index}"]`); const field = select && select.value;
      if (!field || year.enabledPapers.includes(field) || !HelpDeskYearPyqs.paperFields.includes(field)) return;
      year.enabledPapers.push(field); state.selection.yearPyqId = year.id; markDirty(); renderResources();
    }));
  ...
  function renderMeta() {
    ...
    const logoUrl=meta.whatsappLogoUrl||"/images/whatsapp-logo.png";
    editor.innerHTML=`<div class="section-intro"><div><h2>Site identity</h2><p class="muted">Edit the public title, introduction and community links.</p></div></div><section class="panel form-section"><div class="grid">${input("Website title","title",meta.title,{required:true})}${input("Institution","institution",meta.institution,{required:true})}${input("Description","description",meta.description,{type:"textarea",full:true,required:true})}${input("Last updated","lastUpdated",meta.lastUpdated||"",{type:"date"})}</div></section><section class="panel form-section"><div><p class="eyebrow">Hamburger menu</p><h3>Holiday list</h3><p class="muted">Visitors go straight to this Google Drive link when they click Holiday list. Leave it empty to show Coming soon.</p></div><div class="grid">${input("Holiday list — Google Drive link","holidayListUrl",meta.holidayListUrl||"",{type:"url",full:true,help:"Paste the share link and enable Anyone with the link → Viewer in Google Drive. Save draft, then Publish changes."})}</div></section><section class="panel form-section community-settings"><div><p class="eyebrow">Community</p><h3>WhatsApp group</h3><p class="muted">Leave this empty to show “WhatsApp group coming soon.” Add the invite link later to activate the Join button automatically.</p></div><div class="grid">${input("WhatsApp group invite URL","whatsappGroupUrl",meta.whatsappGroupUrl||"",{type:"url",full:true,help:"Use a complete https://chat.whatsapp.com/… invite link."})}</div></section><section class="panel form-section community-logo-settings"><div><p class="eyebrow">Brand icon</p><h3>WhatsApp logo</h3><p class="muted">Upload a PNG, JPEG or WebP image. It updates the community banner and WhatsApp contact icons after Save draft and Publish changes.</p></div><div class="community-logo-editor"><img class="community-logo-preview" data-whatsapp-logo-preview src="${escape(photoSource(logoUrl))}" alt="WhatsApp logo preview" width="96" height="96">${urlInput("Logo image","whatsappLogoUrl",logoUrl,".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp")}<button class="mini-button" id="reset-whatsapp-logo" type="button">Use default logo</button></div></section>`;
    bind(editor,meta);
    const logoInput=$('[data-bind="whatsappLogoUrl"]',editor);const preview=$('[data-whatsapp-logo-preview]',editor);
    logoInput?.addEventListener("input",()=>{preview.src=photoSource(logoInput.value.trim()||"/images/whatsapp-logo.png");});
    preview?.addEventListener("error",()=>{const fallback=new URL("/images/whatsapp-logo.png",window.location.origin).href;if(preview.src!==fallback)preview.src=fallback;});
    $("#reset-whatsapp-logo")?.addEventListener("click",()=>{logoInput.value="/images/whatsapp-logo.png";logoInput.dispatchEvent(new Event("input",{bubbles:true}));toast("Default WhatsApp logo selected. Save the draft when ready.");});
  }

  function openSyllabusFileDialog(groupIndex, sectionIndex) {
    const resources=state.data.resources;
    const section=Number.isInteger(groupIndex)&&Number.isInteger(sectionIndex)
      ? resources.syllabusGroups[groupIndex]?.semesters[sectionIndex] : null;
    const modal=document.createElement("div");modal.className="request-modal";
    modal.innerHTML=`<form class="request-modal-card syllabus-file-modal"><div class="quick-modal-head"><div><p class="eyebrow">Syllabus Citadel</p><h2>Add syllabus file</h2><p class="muted">${section?`Add directly inside ${escape(section.title)}.`:"Create a file now and link it to a section later."}</p></div><button class="icon-button" type="button" data-cancel aria-label="Close">×</button></div><label class="field"><span>Display name</span><input name="title" required maxlength="200" placeholder="Example: Paint Technology Core"></label><label class="field"><span>Google Drive link</span><input name="url" type="url" placeholder="https://drive.google.com/…"><small>Leave empty to show Coming soon. You can also upload a PDF later from All syllabus files.</small></label><div class="management-actions"><button class="primary" type="submit">Add syllabus</button><button class="quiet-button" type="button" data-cancel>Cancel</button></div></form>`;
    document.body.appendChild(modal);
    $$('[data-cancel]',modal).forEach((button)=>button.addEventListener("click",()=>modal.remove()));
    $("form",modal).addEventListener("submit",(event)=>{
      event.preventDefault();const form=new FormData(event.currentTarget);const title=String(form.get("title")||"").trim();const url=String(form.get("url")||"").trim();
      if(!title)return;
      if(url){let valid=false;try{const parsed=new URL(url);valid=parsed.protocol==="https:"&&["drive.google.com","docs.google.com"].includes(parsed.hostname)&&!parsed.username&&!parsed.password;}catch{}if(!valid)return toast("Use an HTTPS Google Drive or Google Docs link, or leave it empty for Coming soon.");}
      const item={id:uniqueId(title,resources.syllabi),title,available:Boolean(url),url:url||null};resources.syllabi.push(item);
      if(section&&!section.syllabusIds.includes(item.id))section.syllabusIds.push(item.id);
      modal.remove();markDirty();renderSyllabus();toast(`${title} added to the draft.`);
    });
  }

  function renderSyllabus() {
    const resources=state.data.resources;HelpDeskSyllabus.ensure(resources);
    const byId=new Map(resources.syllabi.map((item)=>[item.id,item]));
    const groups=resources.syllabusGroups.map((group,gIndex)=>`<section class="panel data-group syllabus-admin-group"><div class="data-group-head"><div><h2>${escape(group.title)}</h2><span class="muted">${escape(group.subtitle||"")}</span></div><button class="quiet-button" data-syl-add-section="${gIndex}">＋ Add section</button></div>${group.semesters.map((section,sIndex)=>`<div class="entry-card syllabus-section-card"><div class="entry-head"><div><strong>${escape(section.title)}</strong><small>${section.syllabusIds.length} linked file${section.syllabusIds.length===1?"":"s"}</small></div><div class="syllabus-section-actions"><button class="mini-button" data-syl-file-add="${gIndex}:${sIndex}">＋ New file</button><button class="mini-button" data-syl-link-existing="${gIndex}:${sIndex}">Link existing</button><button class="mini-button danger" data-syl-delete-section="${gIndex}:${sIndex}">Remove section</button></div></div><div class="entry-fields syllabus-section-fields">${input("Section name",`syllabusGroups.${gIndex}.semesters.${sIndex}.title`,section.title,{required:true})}${input("Section subtitle",`syllabusGroups.${gIndex}.semesters.${sIndex}.subtitle`,section.subtitle||"",{full:true,help:"Shown below the section name on the public site."})}</div><div class="subject-list">${section.syllabusIds.map((id)=>{const item=byId.get(id);return item?`<div class="subject-row"><span><strong>${escape(item.title)}</strong><small>${item.available&&item.url?"Linked":"Coming soon"}</small></span><button class="mini-button danger" data-syl-unlink="${gIndex}:${sIndex}:${escape(id)}" aria-label="Remove ${escape(item.title)} from ${escape(section.title)}">×</button></div>`:"";}).join("")||'<p class="resource-empty">No syllabus files in this section yet.</p>'}</div></div>`).join("")}</section>`).join("");
    const files=resources.syllabi.map((item,index)=>`<div class="entry-card"><div class="entry-head"><div><strong>${escape(item.title)}</strong><small>${escape(item.id)}</small></div><button class="mini-button danger" data-syl-delete="${index}">Delete file</button></div><div class="entry-fields">${input("Title",`syllabi.${index}.title`,item.title,{required:true})}${urlInput("Google Drive or PDF URL",`syllabi.${index}.url`,item.url||"")}<label class="field"><span>Public status</span><select data-syl-available="${index}"><option value="true" ${item.available?"selected":""}>Available</option><option value="false" ${!item.available?"selected":""}>Coming soon</option></select><small class="field-state"></small></label></div></div>`).join("");
    editor.innerHTML=`<div class="section-intro"><div><h2>Syllabus Citadel</h2><p class="muted">Manage Engineering, Technology and Core syllabus sections. Save a draft first; publish only when everything is ready.</p></div><button class="quiet-button" id="add-syllabus">＋ New unlinked file</button></div><div class="collection-stack">${groups}</div><section class="panel data-group syllabus-file-library"><div class="data-group-head"><div><h2>All syllabus files</h2><span class="muted">Edit names, upload PDFs and paste Google Drive links.</span></div></div>${files}</section>`;
    bind(editor,resources);
    ...
    $$('[data-bind^="syllabi."][data-bind$=".url"]').forEach((node)=>node.addEventListener("input",()=>{const index=Number(node.dataset.bind.split(".")[1]);const item=resources.syllabi[index];item.available=Boolean(node.value.trim());const select=$(`[data-syl-available="${index}"]`,editor);if(select)select.value=String(item.available);}));
    $$('[data-syl-unlink]').forEach((node)=>node.addEventListener("click",()=>{const [g,s,id]=node.dataset.sylUnlink.split(":");const section=resources.syllabusGroups[Number(g)].semesters[Number(s)];section.syllabusIds=section.syllabusIds.filter((value)=>value!==id);markDirty();renderSyllabus();}));
    $$('[data-syl-file-add]').forEach((node)=>node.addEventListener("click",()=>{const [g,s]=node.dataset.sylFileAdd.split(":").map(Number);openSyllabusFileDialog(g,s);}));
    $$('[data-syl-link-existing]').forEach((node)=>node.addEventListener("click",()=>{const [g,s]=node.dataset.sylLinkExisting.split(":").map(Number);const section=resources.syllabusGroups[g].semesters[s];const choices=resources.syllabi.filter((item)=>!section.syllabusIds.includes(item.id));if(!choices.length)return toast("Every syllabus file is already linked here.");const id=prompt(`Enter the file ID to link:\n${choices.map((item)=>`${item.id} — ${item.title}`).join("\n")}`);if(id&&byId.has(id)&&!section.syllabusIds.includes(id)){section.syllabusIds.push(id);markDirty();renderSyllabus();}}));
    $$('[data-syl-add-section]').forEach((node)=>node.addEventListener("click",()=>{const group=resources.syllabusGroups[Number(node.dataset.sylAddSection)];const title=prompt("New section name",group.id==="technology"?"Core":"New section");if(!title)return;const id=uniqueId(`${group.id}-${title}`,group.semesters);group.semesters.push({id,title:title.trim(),subtitle:"",syllabusIds:[]});markDirty();renderSyllabus();}));
    $$('[data-syl-delete-section]').forEach((node)=>node.addEventListener("click",()=>{const [g,s]=node.dataset.sylDeleteSection.split(":").map(Number);const group=resources.syllabusGroups[g];const section=group.semesters[s];if(requireDelete(`${section.title} syllabus section`)){group.semesters.splice(s,1);markDirty();renderSyllabus();}}));
    $("#add-syllabus").addEventListener("click",()=>openSyllabusFileDialog());
    $$('[data-syl-delete]').forEach((node)=>node.addEventListener("click",()=>{const index=Number(node.dataset.sylDelete);const item=resources.syllabi[index];if(requireDelete(`${item.title} syllabus file from every section`)){resources.syllabi.splice(index,1);resources.syllabusGroups.forEach((group)=>group.semesters.forEach((section)=>{section.syllabusIds=section.syllabusIds.filter((id)=>id!==item.id);}));markDirty();renderSyllabus();}}));
  }

  function renderCreators() {
    const creators=state.data.resources.creators;
    editor.innerHTML=`<div class="section-intro"><div><h2>Creator profiles</h2><p class="muted">Upload a photo, click Save draft, then Publish changes. The preview below is your draft; publishing makes it visible on the website.</p></div><button class="quiet-button" id="add-creator">＋ Add creator</button></div><div class="collection-stack">${creators.map((creator,index)=>`<section class="panel creator-card"><img class="creator-photo" data-creator-photo="${index}" src="${escape(photoSource(creator.photoUrl))}" alt="${escape(creator.name)} preview"><div class="creator-fields">${input("Name",`${index}.name`,creator.name,{required:true})}${input("Role",`${index}.role`,creator.role,{required:true})}${input("WhatsApp number",`${index}.whatsapp`,creator.whatsapp,{help:"Include country code, digits only."})}${urlInput("Profile photo",`${index}.photoUrl`,creator.photoUrl,"image/png,image/jpeg,image/webp")}${input("Instagram profile URL",`${index}.instagramUrl`,creator.instagramUrl||"",{type:"url",full:true,help:"Optional — hidden from the public site until a link is added."})}${input("LinkedIn profile URL",`${index}.linkedinUrl`,creator.linkedinUrl||"",{type:"url",full:true,help:"Optional — hidden from the public site until a link is added."})}</div><button class="danger-button" data-delete-creator="${index}">Remove</button></section>`).join("")}</div>`;
    bind(editor,creators);
    creators.forEach((creator,index)=>{const photoInput=$(`[data-bind="${CSS.escape(`${index}.photoUrl`)}"]`,editor);const preview=$(`[data-creator-photo="${index}"]`,editor);if(photoInput&&preview){photoInput.addEventListener("input",()=>{preview.src=photoSource(photoInput.value.trim());});preview.addEventListener("error",()=>{if(preview.src.endsWith("/favicon.svg"))return;preview.src="/favicon.svg";});}});
  ...
  function renderManagement() {
    ...
    editor.innerHTML=`<div class="section-intro"><div><h2>Access & approvals</h2><p class="muted">Approve changes into a private draft, then publish them through Blobs without a production deployment.</p></div><span class="role-pill">Main admins only</span></div><div class="access-grid">
      ...
      <section class="panel data-group"><div class="data-group-head"><h2>Main admins</h2><span class="status-pill active">Full access</span></div>${data.mainAdmins.map(admin=>`<div class="entry-card admin-person-row"><img src="${escape(photoSource(admin.photoUrl))}" alt="" width="44" height="44"><div><strong>${escape(admin.name)}</strong><div class="person-meta"><span>@${escape(admin.username)}</span><span>Everything</span>${admin.promoted?`<span>Promoted by ${escape(admin.promotedBy||"main admin")}</span>`:""}</div></div></div>`).join("")}</section>
